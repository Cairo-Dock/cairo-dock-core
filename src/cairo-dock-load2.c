...
void cairo_dock_load_drop_indicator (gchar *cImagePath, cairo_t* pSourceContext, double fMaxScale)
{
	...
	/**if (g_bUseOpenGL && g_pDropIndicatorSurface != NULL)
	{
		...
		if (!gdk_gl_drawable_gl_begin (pGlDrawable, pGlContext))
			return ;
		g_iDropIndicatorTexture = cairo_dock_create_texture_from_surface (g_pDropIndicatorSurface);
		
		double fWidth=0, fHeight=0;
		
		gchar *cGradationTexturePath = g_strdup_printf ("%s/textures/%s", CAIRO_DOCK_SHARE_DATA_DIR, "texture-bilinear-gradation.png");
		cairo_surface_t *pGradationSurface = cairo_dock_create_surface_from_image (cGradationTexturePath,
			...
			&fWidth, &fHeight,
			NULL, NULL);
		g_iBilinearGradationTexture = cairo_dock_create_texture_from_surface (pGradationSurface);
