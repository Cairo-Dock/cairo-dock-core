...
gboolean cairo_dock_render_dock (gpointer pUserData, CairoDock *pDock)
{
...
void cairo_dock_on_change_icon (Icon *pLastPointedIcon, Icon *pPointedIcon, CairoDock *pDock)
{
	...
	if (pPointedIcon != NULL && pDock->render_opengl != NULL && ! CAIRO_DOCK_IS_SEPARATOR (pPointedIcon))
	{
		gboolean bStartAnimation = FALSE;
		cairo_dock_notify (CAIRO_DOCK_ENTER_ICON, pPointedIcon, pDock, &bStartAnimation);
		//if (g_iconEffectRenderer.init != NULL)
		//	pPointedIcon->bOnMouseOverAnimating = g_iconEffectRenderer.init (pPointedIcon, pDock, CAIRO_DOCK_GL_ANIMATION_DT);
		
		if (pDock->iSidGLAnimation == 0 && bStartAnimation)
			pDock->iSidGLAnimation = g_timeout_add (CAIRO_DOCK_GL_ANIMATION_DT, (GSourceFunc) _cairo_dock_gl_animation, pDock);
	}
}
...
gboolean cairo_dock_notification_click_icon (gpointer pUserData, Icon *icon, CairoDock *pDock, guint iButtonState)
{
	...
	{
		{
			if (cairo_dock_get_active_xwindow () == icon->Xid && g_bMinimizeOnClick)  // ne marche que si le dock est une fenêtre de type 'dock', sinon il prend le focus.
...
void on_drag_data_received (GtkWidget *pWidget, GdkDragContext *dc, gint x, gint y, GtkSelectionData *selection_data, guint info, guint t, CairoDock *pDock)
{
	...
	g_print ("%x;%x;%.2f;%x\n", cReceivedData, pPointedIcon, fOrder, CAIRO_CONTAINER (pDock));
...
gboolean cairo_dock_notification_drop_data (gpointer pUserData, const gchar *cReceivedData, Icon *icon, double fOrder, CairoContainer *pContainer)
{
	...
	g_print ("%x;%x;%.2f;%x\n", cReceivedData, icon, fOrder, pContainer);
	
	if (! CAIRO_DOCK_IS_DOCK (pContainer))
		return CAIRO_DOCK_LET_PASS_NOTIFICATION;
	g_print ("%s (%d)\n", __func__, pContainer->iType);
			...
			if (fOrder == CAIRO_DOCK_LAST_ORDER)  // on a lache dessus.
			{
				if (CAIRO_DOCK_IS_LAUNCHER (icon))
				{
					if (CAIRO_DOCK_IS_URI_LAUNCHER (icon))
					{
						...
						{
							g_print ("on le lache sur un repertoire ou un point de montage\n");
							cairo_dock_fm_move_into_directory (cReceivedData, icon, pContainer);
							return CAIRO_DOCK_INTERCEPT_NOTIFICATION;
						}
						else  // on le lache sur un fichier.
						{
							g_print ("on le lache sur un fichier\n");
							return CAIRO_DOCK_LET_PASS_NOTIFICATION;
						}
					}
					...
					{
						g_print ("on le lache sur un lanceur\n");
				...
				{
					g_print ("on le lache sur autre chose qu'un lanceur\n");
					return CAIRO_DOCK_LET_PASS_NOTIFICATION;
				}
			}
