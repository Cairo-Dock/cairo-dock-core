...
typedef enum {
	NB_NOTIFICATIONS_EXT_TOPLEVEL_MANAGER = NB_NOTIFICATIONS_WAYLAND_WM_MANAGER
} CairoExtToplevelNotifications;
...
{
	uint32_t list_id, state_id, list_version, state_version;
	
	const GldiWaylandProtocolInfo *info = gldi_wayland_get_global (ext_foreign_toplevel_state_manager_v1_interface.name);
	if (!info) return FALSE;
	state_id = info->id;
	state_version = info->version;
	...
	// signals
	gldi_object_install_notifications (&myExtToplevelObjectMgr, NB_NOTIFICATIONS_EXT_TOPLEVEL_MANAGER);
	// parent object
